...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO silver.products (
            productkey, productname, brand, color, unitcostusd, unitpriceusd, 
        ...
        SELECT 
            ...
            END AS unitpriceusd,
            CASE 
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '$3' THEN CAST('0802' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) IN ('899.99','184.97','099.99','592.20') THEN CAST('0304' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '$32.00' THEN CAST('0702' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) IN ('$72.66','$73.12') THEN CAST('0306' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) IN ('000.00','620.00','600.00','580.00','560.00','530.00','520.00','500.00','030.00') THEN CAST('0402' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) IN ('989.90','989.00','818.00','818.90','652.90','599.90','599.00','652.00') THEN CAST('0801' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '499.00' THEN CAST('0305' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '475.00' THEN CAST('0804' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) IN ('299.00','199.00','099.00') THEN CAST('0301' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '295.00' THEN CAST('0305' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '109.00' THEN CAST('0203' AS INT)
                WHEN TRIM(REPLACE(subcategorykey, '"', '')) = '650.00' THEN CAST('0805' AS INT)
                ELSE CAST(TRIM(REPLACE(subcategorykey, '"', '')) AS INT)
